...
class _XrCameraFeedManager:
    ...
    def __init__(
        ...
        try:
            ...
            for cfg, (camera, fallback_image) in zip(resolved_cfgs, bound_feeds, strict=True):
                ...
                try:
                    image = image_source.get_image(fallback_image) if image_source is not None else fallback_image
    ...
    def _publish_feed(self, feed: _ActiveFeed) -> None:
        image = (
            feed.image_source.get_image(feed.fallback_image) if feed.image_source is not None else feed.fallback_image
        )
